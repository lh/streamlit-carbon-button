...
with col2:
    if carbon_button(
        "", key="open_file", icon=CarbonIcons.DOCUMENT, button_type="secondary"
    ):
        st.info("Open file")
    st.caption("Open")
...
with col4:
    if carbon_button(
        "", key="copy_file", icon=CarbonIcons.COPY, button_type="secondary"
    ):
        st.info("Copied")
    st.caption("Copy")
...
with col5:
    if carbon_button(
        "", key="settings", icon=CarbonIcons.SETTINGS, button_type="ghost"
    ):
        st.info("Settings")
    st.caption("Settings")
...
with col6:
    if carbon_button(
        "", key="delete_file", icon=CarbonIcons.DELETE, button_type="danger"
    ):
        st.error("Deleted")
    st.caption("Delete")
...
with col2:
    ...
    with c2:
        if carbon_button(
            "", key="stop", icon=CarbonIcons.CLOSE, button_type="secondary"
        ):
            st.info("Stopped")
        st.caption("Stop")
    ...
    with c4:
        if carbon_button(
            "", key="pause", icon=CarbonIcons.HELP, button_type="secondary"
        ):
            st.info("Paused")
        st.caption("Pause")
...
with col2:
    ...
    with cols[1]:
        if carbon_button(
            "", key="delete_icon", icon=CarbonIcons.DELETE, button_type="danger"
        ):
            st.error("Deleted!")
        st.caption("Delete")
...
for i, (col, (btn_name, btn_type, icon)) in enumerate(zip(cols, button_configs)):
    with col:
        ...
        if carbon_button(
            "", key=f"icon_type_{i}", button_type=btn_type, icon=icon, is_default=is_def
        ):
